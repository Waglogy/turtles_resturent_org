<template>
  ...
  <div class="wave">
    ...
    <section class="py-16 px-6 sm:px-12 lg:px-24  text-white">
    <h1 class="text-4xl font-pacifico mb-8 text-center">Menu Highlights</h1>
    <div class="relative">
      <div class="overflow-hidden" ref="sliderContainer">
        <div class="flex transition-transform duration-300 ease-in-out" :style="{ transform: `translateX(-${currentSlides * 100}%)` }">
          <div v-for="item in menuHighlights" :key="item.id" class="flex-shrink-0 w-full sm:w-1/2 lg:w-1/4 px-4">
  ...
  <div class="max-w-4xl mx-auto text-center relative z-10 flex flex-col sm:flex-row items-center sm:items-start sm:justify-between">
   ...
   <section class="py-16 px-6 sm:px-12 lg:px-24 bg-beige-100 ">
      ...
      <p class="text-2xl font-pacifico text-olive-green mt-8 text-center">"Come for the food, stay for the vibes."</p>
    </section>
    ...
    <div class="max-w-3xl mx-auto">
      <div v-for="faq in faqs" :key="faq.id" class="mb-6 border-b pb-4">
        <div 
...
const sliderContainer = ref(null); // Reference to the slider container
const menuSlideInterval = ref(null); // Reference to the slideshow interval (renamed)
...
onMounted(() => {
  ...
  // Add event listeners for pausing and resuming the slideshow
...
onUnmounted(() => {
  ...
  if (sliderContainer.value) {
    sliderContainer.value.removeEventListener('mouseenter', stopSlideShow);
    sliderContainer.value.removeEventListener('mouseleave', startSlideShow);
  }
});
  ...
  const faqs = [
    ...
    { id: 8, question: 'Is it a pet-friendly restaurant?', answer: 'Absolutely! We welcome pets with open arms.' },
    ...
    { id: 10, question: 'What makes Turtles a must-visit spot in Morjim Beach?', answer: 'Just a shell’s throw away from the turtle beach (where Chapora river meets the sea). With a variety of cuisines, signature cocktails, delicious food, good music, and laid-back chill vibes, it’s the perfect spot to relax, dine and soak in the Goan spirit.' },
    { id: 11, question: 'How can I reach Turtles Restaurant & Bar from Morjim Beach?', answer: 'We’re a short walk from Morjim Beach. Use Google Maps to navigate.' },
  ];
